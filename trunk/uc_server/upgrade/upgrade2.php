...
/*
	...
	$Id: upgrade1.php 12117 2008-01-11 06:25:08Z heyond $ - Tranlate by JaJa. http://code.google.com/p/discuzviet/
*/
...
if(file_exists($lock_file) && $action != 'upgsecques') {
	showheader();
	showerror('Upgrading is locked, it should be is already upgraded, and if the data has been restored manually to delete<br />'.str_replace(UC_ROOT, '', $lock_file).'<br />After, the refresh the page again');
	showfooter();
}
...
?>
	...
	<p>This program is used to upgrade UCenter 1.0 to UCenter 1.51</p>
	<p>Before running the upgrade, make sure UCenter 1.51 has been uploaded all the files and directories</p>
	<p>Strongly recommended that you back up your database before upgrading</p>
	<p><a href="<?php echo $PHP_SELF;?>?action=db">If you have to confirm that you complete this procedure, please click here to upgrade</a></p>
...
<?php
	...
	echo "<h4>Short message data have been processed</h4>";
	...
	if(!$total || $total <= $start) {
		...
		@touch($lock_file);
		if($forward) {
			echo "<br /><br /><br /><a href=\"$forward\">The browser will automatically jump page, without human intervention. Unless a long time when your browser does not support frames, please click here</a>";
		...
		$end = $start + $limit;
		echo "Short message data have been processed $start / $total ...";
		$url_forward = "upgrade2.php?action=pm&start=$end&total=$total&forward=".urlencode($forward);
		echo "<br /><br /><br /><a href=\"$url_forward\">The browser will automatically jump page, without human intervention. Unless a long time when your browser does not support frames, please click here</a>";
	...
	if(file_exists($lock_file)) {
		showheader();
		showerror('Upgrading is locked, it should be is already upgraded the security question, if you have to restore the data manually delete<br />'.str_replace(UC_ROOT, '', $lock_file).'<br />After the refresh the page again');
	}
	...
	if(empty($uc_authcode) || authcode($uc_authcode, 'DECODE', UC_KEY) != UC_FOUNDERPW) {
		...
		if(empty($uc_founderpw) || UC_FOUNDERPW !=  md5(md5($uc_founderpw).UC_FOUNDERSALT)) {
			...
			echo 'Please enter your password UCenter Founder:<input type="password" name="uc_founderpw" /> <input type="submit" value="Submit" />';
			exit;
		} else {
	...
	if(!is_dir(UC_ROOT.'./data/upgsecques')) {
		showheader();
		showerror('Please, in the forum ./forumdata/upgsecques directories are uploaded to the UCenter directory ./data/ .Next, followed by <a href="javascript:location.reload();" target="_self">Refresh this page</a>');
	}
	$num = getgpc('num');
	...
	if(!file_exists($dump_file)) {
		...
		showheader();
		echo 'Security question the upgrade is complete, thank you for using this program';
	} else {
		showheader();
		...
		runquery($sql);
		$num++;
		echo "Security question importing";
...
<title>UCenter Upgrade program( $version_old &gt;&gt; $version_new)</title>
...
UCenter Upgrade program( $version_old &gt;&gt; $version_new)</td>
</tr>
<tr>
<td>
...
<tr>
<td align="center">
<b>The upgrade process can only $version_old to $version_new make sure all the files have been uploaded, and make data backup<br />
Upgrading these have any questions please visit the Technical Support<a href="http://code.google.com/p/discuzviet/" target="_blank">http://code.google.com/p/discuzviet/</a></b>
</td>
</tr>
<tr>
<td>
...
<br /><br />
&gt;&gt;<a href="$url">The browser will automatically jump page, without human intervention. Unless a long time when your browser does not support frames, please click here</a>
<br /><br />
EOT;
	showfooter();
}
...
function authcode($string, $operation = 'DECODE', $key = '', $expiry = 0) {
	...
	$ckey_length = 4;	//random key length 0-32;
					//join a random key, you can order the ciphertext is no law, even when the original text and key results in exactly the same, encryption and will also be different each time, increase the deciphering of the more difficult.
					//evaluates the bigger the ciphertext variations greater the ciphertext change $ckey_length = 16
					//When this value is 0, it does not produce random key
